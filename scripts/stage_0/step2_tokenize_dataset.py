"""
WikiForge-GPT Dataset Tokenization (Memory-Efficient Version)
==============================================================
Tokenize the full Wikipedia dataset using OUR custom tokenizer.
...
class MemoryEfficientTokenizer:
    ...
    Processes and saves data incrementally.
    """
    ...
    def __init__(
        ...
        self.output_dir = PATHS.TRAINING_DATA / f"stage_0_tiny_vocab{vocab_size}"
    ...
    def process_split(self, split_name: str) -> Dict:
        ...
        output_file = self.output_dir / f"{split_name}_tokenized.npy"
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("🚀 WikiForge-GPT Dataset Tokenization (Memory-Efficient)")
        print("="*80)
        ...
        print(f"\n💡 Next Steps:")
        print(f"   1. Review tokenized data in {self.output_dir}")
        print(f"   2. Proceed to Step 3: Build GPT architecture")
        print(f"   3. Start Stage 0 training!")
...
def main():
    ...
    # Stage 0: Tiny model configuration
    print("\n🎯 Tokenizing for Stage 0 (Tiny Model)")
    print("   Vocab: 8K | Sequence Length: 256 tokens")
    ...
    tokenizer = MemoryEfficientTokenizer(
        ...
        max_seq_length=256,
        stride=128,
        chunk_size=10000,  # 10K articles per chunk = ~500MB RAM
    )
