"""
STAGE_3 - Step 3: Train 350M Model (FULLY FIXED & ROBUST)
===============================================================================
Architecture: 16 layers, 16 heads, 1024 d_model
Expected time: ~240 hours (10 days)
Expected loss: ~1.7

ALL FIXES APPLIED:
✅ Proper data loading (np.memmap)
✅ Correct model call (labels= keyword)
✅ Updated torch.amp API
...
def load_data(data_path, seq_length=256):
    """
    Load data using memory-mapped binary (PROVEN method from Stage 1).
    """
...
def train_model():
    print("\n" + "="*80)
    print("STAGE_3 - TRAIN 350M MODEL (MAXIMUM!)")
    print("="*80 + "\n")
    ...
    print(f"  Model: 350M parameters")
    print(f"  Architecture: 16 layers, 16 heads, 1024 d_model")
    ...
    data_dir = PATHS.TRAINING_DATA / "stage_3_vocab50000"
    ...
    # Setup directories
    checkpoint_dir = PATHS.CHECKPOINTS / "stage_3" / "checkpoints"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    
    log_dir = PATHS.LOGS / "training"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "stage_3_training_log.jsonl"
    ...
    while step < max_steps:
        ...
        for batch_start in range(0, len(indices), batch_size):
            ...
            # Forward pass with mixed precision (FIXED: use labels= keyword)
            with autocast('cuda'):
